...
const CSVUploader = () => {
  ...
  const parseCSVToFindings = (csvData: string): AuditFinding[] => {
    ...
    return dataRows.map((line, index) => {
      ...
      // Expect more columns for extended data
      // Expected format: auditRef, clause/reference, finding, observation, year, date(optional)
      if (columns.length < 5) {
      ...
      return {
        ...
        auditRef: columns[0],
        checklistItemId: columns[1], // This can be adapted later
        clause: columns[1], // Also store the clause for matching
        reference: columns[1], // Also store as reference for matching
        finding: columns[2],
        observation: columns[3],
        year: columns[4],
        date: columns.length > 5 ? columns[5] : undefined,
        isHistorical: true
      };
    });
  };
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-xs text-gray-500 mt-1">
          The CSV should contain: auditRef, clause, finding, observation, year, date(optional)
        </p>
        <input
          type="file"
          ref={fileInputRef}
